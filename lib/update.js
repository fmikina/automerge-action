...
async function update(context, dir, url, pullRequest) {
  ...
  let isReviewedCorrectly = await isPrReviewed(octokit, pullRequest);
  if (!isReviewedCorrectly) {
    ...
    logger.info("Pr review is correct (approved and has no comments)")
  }
...
async function dropAutoMergeLabel(octokit, pullRequest, actions) {
  ...
  const { data: comment } = await octokit.repos.createCommitComment({
    owner: pullRequest.head.repo.owner.login,
    repo: pullRequest.head.repo.name,
...
async function isPrReviewed(octokit, pullRequest) {
  ...
  for (const review of reviews) {
    if (review.state == "CHANGES_REQUESTED") {
      ...
      return foundDroppedLabel || !comments || !comments.length;
    } else if (review.body == AUTOMERGE_LABEL_DROPPED) {
      foundDroppedLabel = true;
    }
  }
