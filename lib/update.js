...
const AUTOMERGE_LABEL_DROPPED = 'Automerge label was dropped due to the fact that either someone requested changes' +
                                ', you do not have a review, or someone who reviewed and approved your pr left comments. ' +
                                'To force automerge now you need to add \`automerge\` or \`autorebase\` label once again.';
...
async function update(context, dir, url, pullRequest) {
  ...
  if (!skipAdvancedApprovalValidation) {
    ...
    if (!isReviewedCorrectly) {
      logger.info("Pr review state is unstable (either requested changes or approve has comments)")
      await dropAutoMergeLabel(octokit, pullRequest, actions);
      throw new NeutralExitError();
    } else {
      logger.info("Pr review is correct (approved and has no comments) or empty (will check merging afterwards)")
    }
...
async function isPrReviewed(octokit, pullRequest, actions) {
  ...
  for (const review of reviews) {
    if (review.state == "CHANGES_REQUESTED" || review.state == "APPROVED") {
      ...
      events.reverse();
      let labeledAfterLastApproval = false;
      for (const event of events) {
        if (event.event == "labeled" && actions.includes(event.label.name)) {
          var eventDate = new Date(event.created_at);
          var reviewDate = new Date(review.submitted_at);

          labeledAfterLastApproval = (reviewDate < eventDate)
          logger.info("last review time", review.submitted_at, "last labelled time", event.created_at, "labelled after", labeledAfterLastApproval);

        }
      }

      return labeledAfterLastApproval || !comments || !comments.length;
    }
  }
